(function () {
  ...
  function objectLooksLikeAd(value) {
    ...
    try {
      ...
      for (var i = 0; i < keys.length; i++) {
        if (isAdKey(keys[i]) || (typeof value[keys[i]] === 'string' && isAdUrl(value[keys[i]]))) return true;
      }
    } catch (e) {}
    return false;
  }
  ...
  function cleanAds(value, depth) {
    ...
    if (Array.isArray(value)) {
      for (var i = value.length - 1; i >= 0; i--) {
        if ((typeof value[i] === 'string' && isAdUrl(value[i])) || objectLooksLikeAd(value[i])) value.splice(i, 1);
  ...
  function markNode(node) {
    ...
    if (node.classList) node.classList[shouldHide ? 'add' : 'remove']('lampa-adblock-hidden');
  }
  ...
  function patchDom() {
    ...
    new MutationObserver(function (changes) {
      ...
      subtree: true,
      attributes: true,
      attributeFilter: ['src', 'href', 'data-src', 'class', 'id', 'data-ad', 'data-ads', 'data-advert', 'data-banner']
    });
  }
